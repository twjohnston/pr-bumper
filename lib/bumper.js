...
class Bumper {
  ...
  bump () {
    ...
    return this.ci.getLastCommitMsg()
      ...
      .then((info) => {
        return this._maybeBumpManifest(info)
      })
      .then((info) => {
  ...
  _getMergedPrInfo () {
    return this._getLastPr()
      .then((pr) => {
        ...
        if (this.config.isEnabled('manifest')) {
          manifestScopes = utils.getManifestScopesForPr(pr)
        }
        ...
        if (manifestScopes) {
          info.manifestScopes = manifestScopes
        }
  ...
  _maybeBumpManifest (info) {
    const {manifestScopes} = info

    if (!this.config.isEnabled('manifest') || !manifestScopes) {
      return info
    }

    const manifestFile = this.config.features.manifest.file
    ...
    manifestScopes.forEach(({component, scope}) => {
      const currentVersion = manifestData[component]

      if (currentVersion) {
        const v = versiony.version(currentVersion)
        switch (scope) {
          case 'patch':
            v.patch()
            break
        ...
        manifestData[component] = v.get()
      } else {
        throw new Error(`Component ${component} not found in manifest file.`)
      }
    })

    fs.writeFileSync(manifestFile, JSON.stringify(manifestData, null, 2))
    addModifiedFile(info, manifestFile)
